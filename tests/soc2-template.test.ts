...
test("consultant SOC 2 template preserves the complete 38-criterion mapping", () => {
...
test("every SOC 2 criterion is audit-ready and traceable", () => {
  for (const control of soc2TemplateControls) {
    ...
    assert.ok(control.performingControls.length > 10, `${control.tscId} performing control is missing`);
    ...
    assert.ok(["Uygun", "Kısmi", "Eksik"].includes(control.consultantStatus));
    assert.match(control.source, /^https:\/\//);
  }
});
