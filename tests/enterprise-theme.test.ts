...
test("blue is the interaction color and orange is retired",()=>{assert.match(css,/--atlas-brand:#3574e6/);assert.match(css,/--atlas-brand:#6ca8ff/);assert.doesNotMatch(css,/#(?:d84b20|ff7849|ff9a76)/i);});
test("atlas is application-owned CSS without a Tailwind runtime",()=>{assert.doesNotMatch(packageJson,/@tailwindcss|"tailwindcss"/);assert.match(css,/Fornost Atlas/);});
