...
export async function POST(req:NextRequest){
 ...
 const {env}=await import("cloudflare:workers");if(!env.BUCKET)return NextResponse.json({error:"Dosya alanı hazır değil"},{status:503});
...
export async function GET(req:NextRequest){
 ...
 if(!key||!key.startsWith("evidence/")||key.includes("..")||!env.BUCKET)return new NextResponse("Bulunamadı",{status:404});
 const obj=await env.BUCKET.get(key);if(!obj)return new NextResponse("Bulunamadı",{status:404});
 const type=obj.httpMetadata?.contentType||"application/octet-stream",inline=req.nextUrl.searchParams.get("inline")==="1"&&type.startsWith("image/");
 return new NextResponse(obj.body,{headers:{"content-type":type,"content-disposition":`${inline?"inline":"attachment"}; filename=\"${key.split("/").pop()}\"`,"x-content-type-options":"nosniff","cache-control":"private, no-store","content-security-policy":"default-src 'none'; img-src 'self' data:; style-src 'unsafe-inline'"}});
}
