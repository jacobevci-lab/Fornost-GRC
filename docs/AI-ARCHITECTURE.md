...
Migration `0065_fornost_ai_regulatory_obligations.sql` operationalizes regulatory profiles as accountable obligations across the EU AI Act, ISO/IEC 42001, NIST AI RMF, KVKK/GDPR and internal policy. Each obligation has a stable framework code, owner, separate reviewer, due date, priority, evidence plan and optional recurrence. Implementation and independent verification require evidence references with SHA-256 integrity; creators and submitters cannot verify their own work. Open High/Critical or overdue obligations fail the regulatory release-gate domain, appear in the executive action queue and are preserved in the assurance dossier. The workflow records references and hashes, not evidence bodies or sensitive model content.
