...
make_case() {
  ...
  printf 'FORNOST_BASE_PATH=%s\nFORNOST_HTTPS_PORT=%s\nFORNOST_SETTINGS_ENCRYPTION_KEY=%s\nFORNOST_DOSSIER_SIGNING_KEY=%s\nFORNOST_SCHEDULER_TOKEN=%s\n' "${base_path}" "${port}" "test-only-settings-encryption-key-0123456789abcdef0123456789abcdef" "test-only-dossier-signing-key-0123456789abcdef0123456789abcdef" "test-only-scheduler-token-0123456789abcdef0123456789abcdef" >"${project_dir}/.env.onprem"
...
grep -q 'podman run .*localhost/fornost-grc-app:1111111111111111111111111111111111111111' "${cache_case}/engine.log" || fail "commit-pinned image tag was not used to start the application"
