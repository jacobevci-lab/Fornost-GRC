...
test("delivery posture counts transport acceptance separately from pending and SLA breaches",()=>{
 const now=new Date("2026-09-21T08:00:00.000Z");
 const pending=outbox(),inApp=outbox({id:"O-2",recipient:"",route:"in-app-governance",severity:"high"});
 const first=summarizeAssuranceDelivery([pending,inApp],[],defaultAssuranceNotificationPolicy,now);
 assert.equal(first.pending,1);assert.equal(first.sent,0);assert.equal(first.inAppOnly,1);assert.equal(first.slaBreaches,1);
 const accepted=summarizeAssuranceDelivery([pending,inApp],[delivery()],defaultAssuranceNotificationPolicy,now);
 assert.equal(accepted.pending,0);assert.equal(accepted.sent,1);assert.equal(accepted.slaBreaches,0);
});

test("failed attempts remain pending and visible as failures",()=>{
 const result=summarizeAssuranceDelivery([outbox({severity:"high"})],[delivery({state:"failed",provider:"smtp-bridge",detail:"HTTP 502"})],defaultAssuranceNotificationPolicy,new Date("2026-09-21T02:00:00.000Z"));
 assert.equal(result.pending,1);assert.equal(result.failed,1);assert.equal(result.sent,0);
});
...
test("notification dispatcher is Admin-only for policy and delivery while posture remains readable",()=>{
 ...
 assert.match(route,/action!=="dispatch"/);
 assert.match(route,/maxAttempts/);
 assert.match(route,/state==="sent"/);
 assert.match(route,/Etkin e-posta entegrasyonu bulunamadı/);
});

test("SLA policy has ordered enterprise defaults and delivery attempts are auditable",()=>{
 const runtime=readFileSync("app/assurance-notification-delivery.ts","utf8");
 const route=readFileSync("app/api/continuous-assurance/notifications/route.ts","utf8");
 ...
 assert.match(runtime,/continuous_assurance_notification_deliveries/);
 assert.match(runtime,/continuous_assurance_notification_policy/);
 assert.match(route,/criticalSlaMinutes<=next\.highSlaMinutes/);
 assert.match(route,/highSlaMinutes<=next\.mediumSlaMinutes/);
