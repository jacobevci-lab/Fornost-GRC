...
describe('IdentityService', () => {
  it('rejects requests without a bearer token', async () => {
    ...
    const prisma = { userIdentity: { findMany: vi.fn() } };
  ...
  it('derives tenant and permissions from the verified membership', async () => {
    const verifier = { verify: vi.fn().mockResolvedValue({ issuer: 'issuer', subject: 'subject' }) };
    const prisma = {
      userIdentity: {
        findMany: vi.fn().mockResolvedValue([{ user: {
          id: 'user-id', tenantId: '11111111-1111-4111-8111-111111111111',
          roles: [{ role: { permissions: ['grc:read'] } }],
        } }]),
      },
    };
    const service = new IdentityService(verifier, prisma as never);
    const principal = await service.authenticate('Bearer signed-token');
  ...
  it('rejects an unverified tenant selector', async () => {
    const verifier = { verify: vi.fn().mockResolvedValue({ issuer: 'issuer', subject: 'subject' }) };
    const prisma = { userIdentity: { findMany: vi.fn().mockResolvedValue([]) } };
    const service = new IdentityService(verifier, prisma as never);
    await expect(service.authenticate('Bearer signed-token', '11111111-1111-4111-8111-111111111111'))
...
describe('AuthorizationGuard', () => {
  it('denies endpoints without an explicit permission policy', () => {
    const reflector = { getAllAndOverride: vi.fn().mockReturnValueOnce(false).mockReturnValueOnce(undefined) };
  ...
  it('requires every declared permission', () => {
    const reflector = { getAllAndOverride: vi.fn().mockReturnValueOnce(false).mockReturnValueOnce(['grc:read', 'risk:write']) };
    const guard = new AuthorizationGuard(reflector as unknown as Reflector);
