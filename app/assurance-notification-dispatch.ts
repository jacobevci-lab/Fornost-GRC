...
export async function dispatchAssuranceNotifications(db:D1Database,env:Env,options:{actor:string;trigger:AssuranceDispatchTrigger;limit?:number;now?:Date}):Promise<AssuranceDispatchResult>{
 ...
 try{
  ...
  const candidates=data.outbox.filter(item=>isCandidate(item,history.get(item.id)||[],policy.maxAttempts,nowMs)).sort((a,b)=>severityRank(a.severity)-severityRank(b.severity)||a.created_at.localeCompare(b.created_at)).slice(0,requestedLimit);candidateCount=candidates.length;
