...
type Lang="tr"|"en";
type Category="control"|"review"|"capa"|"finding"|"risk"|"escalation";
...
const categories:Category[]=["control","review","capa","finding","risk","escalation"];
...
export default function ContinuousAssuranceTimeline({lang}:{lang:Lang}){
 ...
 const label=(value:Category)=>tr?({control:"Kontrol",review:"İnceleme",capa:"CAPA",finding:"Bulgu",risk:"Risk",escalation:"Escalation"} as Record<Category,string>)[value]:({control:"Control",review:"Review",capa:"CAPA",finding:"Finding",risk:"Risk",escalation:"Escalation"} as Record<Category,string>)[value];
 ...
 return <section className="assurance-timeline">
  <header><div><small>ASSURANCE TIMELINE</small><h4>{tr?"Uçtan uca güvence geçmişi":"End-to-end assurance history"}</h4><p>{tr?"Kontrol, inceleme, CAPA, bulgu, risk ve escalation yaşam döngüsünü tek kronolojide izleyin.":"Trace control, review, CAPA, finding, risk and escalation lifecycles in one chronology."}</p></div><span>{events.length}</span></header>
