...
async function markRiskForReview(db:D1Database,riskId:string,exceptionId:string,reason:"expired"|"revoked",stamp:string){
 ...
 await db.prepare("UPDATE simple_grc_records SET data_json=?,updated_at=? WHERE id=?").bind(JSON.stringify(data),stamp,riskId).run();
}
