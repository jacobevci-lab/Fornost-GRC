...
import { assessConnectedGrcCoverage, buildConnectedGrcGraph, connectedRelationLabels, connectedTitle, type ConnectedGrcRow } from "./connected-grc-model";
...
export default function ConnectedGrc({rows,lang,go}:{rows:ConnectedGrcRow[];lang:Lang;go:(module:string)=>void}){
  ...
  return <section className="connected-grc">
    ...
    <section className="connected-assurance" aria-label={tr?"İlişki güvence boşlukları":"Relationship assurance gaps"}>
      <header><div><small>{tr?"SÜREKLİ GÜVENCE":"CONTINUOUS ASSURANCE"}</small><h3>{tr?"İlişki sağlığı":"Relationship health"}</h3></div><p>{tr?`${coverage.covered} / ${coverage.eligible} kritik kayıt, beklenen risk–kontrol–kanıt–denetim zincirine bağlı.`:`${coverage.covered} / ${coverage.eligible} critical records are connected to the expected risk-control-evidence-audit chain.`}</p></header>
      {coverage.gaps.length?<div className="connected-gap-list">{coverage.gaps.slice(0,6).map((gap)=><button key={`${gap.row.module}-${gap.row.id}-${gap.rule}`} onClick={()=>go(gap.row.module)}><span className={gap.severity}>{gap.severity==="high"?(tr?"Yüksek":"High"):(tr?"Orta":"Medium")}</span><b>{gap.row.code||gap.row.id}</b><em>{connectedTitle(gap.row)}</em><small>{tr?({"risk-context":"Varlık veya süreç bağı yok","control-assurance":"Kanıt veya çerçeve bağı yok","evidence-control":"Kontrol bağı yok","audit-traceability":"Risk, kontrol veya kanıt bağı yok"}[gap.rule]):({"risk-context":"Missing asset or process link","control-assurance":"Missing evidence or framework link","evidence-control":"Missing control link","audit-traceability":"Missing risk, control or evidence link"}[gap.rule])}</small></button>)}</div>:<div className="connected-assurance-ok">{tr?"Kritik izlenebilirlik boşluğu bulunmadı.":"No critical traceability gaps found."}</div>}
    </section>
