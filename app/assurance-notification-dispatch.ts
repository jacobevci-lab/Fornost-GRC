...
import {acquireAssuranceRecipientLease,clearAssuranceStormDeferral,ensureAssuranceNotificationStormSchema,readAssuranceNotificationStormPolicy,recipientStormDecision,recordAssuranceStormDeferral,releaseAssuranceRecipientLease} from "./assurance-notification-storm";
...
export async function ensureAssuranceNotificationDispatchSchema(db:D1Database){await ensureAssuranceNotificationDeliverySchema(db);await ensureAssuranceNotificationStormSchema(db);for(const sql of schema)await db.prepare(sql).run()}
...
export async function dispatchAssuranceNotifications(db:D1Database,env:Env,options:{actor:string;trigger:AssuranceDispatchTrigger;limit?:number;now?:Date}):Promise<AssuranceDispatchResult>{
 await ensureAssuranceNotificationDispatchSchema(db);const now=options.now||new Date(),startedAt=now.toISOString(),runId=`CAD-${crypto.randomUUID()}`,requestedLimit=Math.max(1,Math.min(50,Math.floor(options.limit||20))),actor=String(options.actor||"system:assurance-dispatch").slice(0,320),results:AssuranceDispatchResult["results"]=[];let sent=0,failed=0,skipped=0,stormDeferred=0,candidateCount=0,transportConfigured=false;
 await db.prepare("DELETE FROM continuous_assurance_notification_leases WHERE leased_until<=?").bind(startedAt).run();await db.prepare("DELETE FROM continuous_assurance_notification_recipient_leases WHERE leased_until<=?").bind(startedAt).run();
 ...
 try{
  ...
  const [policy,stormPolicy,data]=await Promise.all([readAssuranceNotificationPolicy(db),readAssuranceNotificationStormPolicy(db),readAssuranceDeliveryData(db)]),history=historyByOutbox(data.deliveries),resetMap=latestRetryResetByOutbox(data.retryResets),nowMs=now.getTime();
  ...
  for(const item of candidates){const leaseNow=new Date(),token=await acquireLease(db,item.id,leaseNow);if(!token){skipped++;continue}let recipientKey="",recipientToken="";try{
    ...
    if(item.severity!=="critical"){
      const recipientLease=await acquireAssuranceRecipientLease(db,item.recipient,leaseNow);recipientKey=recipientLease.key;recipientToken=recipientLease.token;if(!recipientToken){skipped++;continue}
      const storm=await recipientStormDecision(db,item.recipient,item.severity,stormPolicy,leaseNow);if(!storm.allowed){await recordAssuranceStormDeferral(db,item.id,item.recipient,item.severity,storm.nextEligibleAt,leaseNow);stormDeferred++;skipped++;continue}
    }
    await clearAssuranceStormDeferral(db,item.id,leaseNow);
   ...
   }finally{await releaseAssuranceRecipientLease(db,recipientKey,recipientToken);await releaseLease(db,item.id,token)}}
  const completedAt=new Date().toISOString(),status:AssuranceDispatchRun["status"]=candidateCount===0?"noop":failed===0?"success":sent>0?"partial":"failed",detail=candidateCount===0?"No eligible notifications were due.":`${sent} transport accepted; ${failed} failed; ${skipped} skipped; ${stormDeferred} storm-deferred.`;
