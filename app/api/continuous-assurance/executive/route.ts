...
async function escalationRows(db:D1Database){try{return (await db.prepare("SELECT id,kind,severity,subject_ref,owner,title,detail,status,first_seen_at,last_seen_at,acknowledged_by,acknowledged_at,resolved_at FROM continuous_assurance_escalations ORDER BY last_seen_at DESC LIMIT 2000").all<AssuranceEscalationRecord>()).results||[]}catch{return []}}

export async function GET(req:NextRequest){
 const access=await requireRole(req,["Admin","Editor","Viewer"]);if(access.response)return access.response;
 const env=await runtime(),settings=await reminderConfig(env.DB),escalations=await escalationRows(env.DB);await ensureAssuranceNotificationSchema(env.DB);await syncAssuranceNotificationOutbox(env.DB,escalations,settings.remindersEnabled);
