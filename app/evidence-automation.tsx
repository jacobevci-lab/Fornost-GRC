...
const controlRefs = (value:string) => value.split(/[;,|\n]+/).map(x=>x.trim()).filter(Boolean);
...
export default function EvidenceAutomation({lang,currentUser}:{lang:Lang;currentUser:{role:string}}){
  ...
  function startPromotion(finding:Finding){
    const mapped=controlRefs(rules.find(x=>x.id===finding.ruleId)?.controlRefs||"");
    setPromotion({finding,controlRef:mapped.length===1?mapped[0]:"",riskRef:"",reviewer:"",dueDate:finding.dueDate||"",rootCause:"",correctiveAction:"",preventiveAction:""});
  }
  async function promoteFinding(e:FormEvent){
    e.preventDefault();if(!promotion)return;
    setBusy("promote-capa");setMessage("");
    try{
      const {finding,...governance}=promotion;
      const r=await fetch(withBasePath("/api/findings/promote-continuous-assurance"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({findingId:finding.id,...governance})});
      const j=await r.json().catch(()=>({})) as {message?:string;error?:string;reasons?:string[]};
      const reasonText=Array.isArray(j.reasons)&&j.reasons.length?` ${j.reasons.join(" · ")}`:"";
      setMessage(r.ok?(j.message||(tr?"Kurumsal CAPA oluşturuldu.":"Enterprise CAPA created.")):`${j.error||(tr?"CAPA aktarımı başarısız.":"CAPA promotion failed.")}${reasonText}`);
      if(r.ok){setPromotion(null);await load()}
    }catch{setMessage(tr?"CAPA servisine ulaşılamadı.":"CAPA service unavailable.")}
    finally{setBusy("")}
  }
  ...
  return <section className="ea-page">
    <div className="ea-hero"><div><small>{tr?"SÜREKLİ GÜVENCE":"CONTINUOUS ASSURANCE"}</small><h2>{tr?"Kanıt Otomasyonu":"Evidence Automation"}</h2><p>{tr?"Her üreticiden API kanıtı topla, kuralla doğrula, kontrol maddelerine bağla ve başarısız sinyali yönetilen CAPA akışına taşı.":"Collect API evidence from any vendor, validate it, map it to controls and promote failed assurance signals into governed CAPA."}</p></div><div className="ea-hero-actions"><span className="ea-live"><i/>{tr?"Collector hazır":"Collector ready"}</span>{currentUser.role==="Admin"&&<button className="primary" onClick={()=>setSourceOpen(true)}>+ {tr?"Kaynak Ekle":"Add Source"}</button>}</div></div>
    ...
    {tab==="findings"&&<DataTable empty={tr?"Açık veya geçmiş kontrol bulgusu yok.":"No current or historical control findings."} heads={[tr?"Bulgu":"Finding",tr?"Önem":"Severity",tr?"Sahip / termin":"Owner / due",tr?"Tekrar":"Occurrences",tr?"Durum":"Status",tr?"Yönetim":"Governance"]} rows={findings.map(x=>[
      ...
      <div key="actions" className="ea-row-actions">
        {x.status!=="closed"&&currentUser.role!=="Viewer"&&<button className="primary" disabled={!!busy} onClick={()=>startPromotion(x)}>{tr?"CAPA'ya Aktar":"Promote to CAPA"}</button>}
    ...
    {promotion&&<Modal title={tr?"Kurumsal CAPA'ya aktar":"Promote to enterprise CAPA"} closeLabel={tr?"Pencereyi kapat":"Close dialog"} onClose={()=>setPromotion(null)}><form className="ea-form" onSubmit={promoteFinding}><div className="wide"><small>{tr?"KAYNAK SÜREKLİ GÜVENCE BULGUSU":"SOURCE CONTINUOUS ASSURANCE FINDING"}</small><b>{promotion.finding.title}</b></div><label>{tr?"Hedef kontrol":"Target control"}<input required value={promotion.controlRef} onChange={e=>setPromotion({...promotion,controlRef:e.target.value})}/></label><label>{tr?"Bağlı risk":"Linked risk"}<input required value={promotion.riskRef} onChange={e=>setPromotion({...promotion,riskRef:e.target.value})}/></label><label>{tr?"Bağımsız reviewer e-posta":"Independent reviewer email"}<input required type="email" value={promotion.reviewer} onChange={e=>setPromotion({...promotion,reviewer:e.target.value})}/></label><label>{tr?"CAPA termini":"CAPA due date"}<input required type="date" value={promotion.dueDate} onChange={e=>setPromotion({...promotion,dueDate:e.target.value})}/></label><label className="wide">{tr?"Kök neden":"Root cause"}<textarea required value={promotion.rootCause} onChange={e=>setPromotion({...promotion,rootCause:e.target.value})}/></label><label className="wide">{tr?"Düzeltici aksiyon":"Corrective action"}<textarea required value={promotion.correctiveAction} onChange={e=>setPromotion({...promotion,correctiveAction:e.target.value})}/></label><label className="wide">{tr?"Önleyici aksiyon":"Preventive action"}<textarea required value={promotion.preventiveAction} onChange={e=>setPromotion({...promotion,preventiveAction:e.target.value})}/></label><div className="wide"><small>{tr?"Kural, otomasyon bulgusu ve SHA-256 kaynak kanıt izi sunucuda doğrulanır; istemciden kabul edilmez.":"Rule, automation finding and SHA-256 source evidence lineage are verified server-side and are not trusted from the client."}</small></div><footer><button type="button" className="ghost" onClick={()=>setPromotion(null)}>{tr?"Vazgeç":"Cancel"}</button><button className="primary" disabled={!!busy}>{tr?"Kurumsal CAPA Oluştur":"Create Enterprise CAPA"}</button></footer></form></Modal>}
