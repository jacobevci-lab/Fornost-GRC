...
export async function POST(req:NextRequest){
  ...
  const operating=await checkAiAccess(env.DB,access.actor.role,"retrieval");if(!operating.allowed){await recordAiEvent(env.DB,{actor:access.actor.email,action:"knowledge-search-policy-denied",status:"denied",detail:operating.code});return json({error:operating.message,code:operating.code},423);}
