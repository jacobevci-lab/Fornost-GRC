...
import { splitRiskAssessmentState } from "./risk-methodology";
...
export default function DashboardV9Executive(){
  ...
  const data=useMemo(()=>{
    ...
    const riskAssessment=splitRiskAssessmentState(risks),riskScores=riskAssessment.assessed.map(row=>row.riskScore),assessedRisks=riskAssessment.assessed.length,unassessedRisks=riskAssessment.unassessed.length,criticalRisks=riskScores.filter(score=>score>=17).length,highRisks=riskScores.filter(score=>score>=10&&score<17).length,avgRisk=riskScores.length?Math.round(riskScores.reduce((sum,score)=>sum+score,0)/riskScores.length):0;
  ...
  const healthMetrics:Metric[]=[
    ...
    {key:"risk",labelTr:"Yüksek / Kritik Risk",labelEn:"High / Critical Risks",value:String(data.highRisks+data.criticalRisks),detailTr:`${data.criticalRisks} kritik · ${data.highRisks} yüksek · ${data.unassessedRisks} değerlendirme bekliyor`,detailEn:`${data.criticalRisks} critical · ${data.highRisks} high · ${data.unassessedRisks} awaiting assessment`,targetTr:"Hedef 0 kritik",targetEn:"Target 0 critical",tone:data.criticalRisks?"critical":data.highRisks||data.unassessedRisks?"watch":"healthy",module:"Risk Assessment"},
  ...
  const outlook:OutlookItem[]=[
    ...
    {key:"risk",count:data.riskReviewsDue30,labelTr:"30 günde risk gözden geçirme",labelEn:"Risk reviews due in 30 days",detailTr:data.unassessedRisks?`${data.unassessedRisks} risk değerlendirme bekliyor`:"Yaklaşan risk sahipliği kararları",detailEn:data.unassessedRisks?`${data.unassessedRisks} risks awaiting assessment`:"Upcoming risk ownership decisions",tone:data.riskReviewsDue30||data.unassessedRisks?"watch":"healthy",module:"Risk Assessment"},
