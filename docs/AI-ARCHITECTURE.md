...
The Admin-only assurance dossier exports a bounded 30, 90 or 365-day model snapshot across twenty-one governance domains. Its JSON manifest contains record references, evidence SHA-256 values, current-state counts, the latest release decision and a canonical SHA-256 digest of the complete payload. An Admin can also seal the snapshot as an immutable `ai_assurance_packages` record using a dedicated `FORNOST_DOSSIER_SIGNING_KEY`; the HMAC-SHA-256 key identifier, signature, package identifier and exact manifest are retained for later server-side verification and audited download. The on-prem installer generates and preserves this key separately from integration encryption material. A formula-safe CSV is also available. Prompt text, model responses, evidence bodies and secrets are excluded; every export, seal, verification and download is appended to the AI activity audit trail. The HMAC proves a matching server key produced an untampered payload and provides a server-origin integrity and authenticity seal; it is not a qualified electronic signature, independent regulatory certification or a substitute for auditor judgment.

Migration `0061_fornost_ai_exceptions.sql` adds time-bound policy, control, risk and release waivers. Requests require a bounded scope, business rationale, accountable owner, compensating controls, review date and a maximum 180-day expiry. Critical-risk exceptions cannot be approved, request creators cannot approve their own records, decisions require exact human confirmation, and every transition is audited. Draft, expired or review-overdue exceptions block the risk domain of the production release gate. Active approved waivers remain visible in the executive portfolio and assurance dossier without being treated as invisible control completion.

Migration `0062_fornost_ai_decommission.sql` replaces direct model retirement with a controlled plan, independent approval, evidenced execution and independent verification. Plans cover downstream dependencies, stakeholder notification, rollback, account/token revocation, data deletion/anonymization/archive or legal hold, signed-model artifacts, monitoring closure and retained proof. Execution and verification require a SHA-256 evidence reference; the executor cannot verify their own work. Only successful verification changes the inventory status to `retired`, preserving the model and assurance records as an auditable history rather than deleting them.

Migration `0063_fornost_ai_findings.sql` adds a source-linked corrective and preventive action register across eighteen AI assurance domains. Every finding carries severity, owner, SLA, root cause, corrective action and recurrence-prevention plan. Workflow transitions are restricted to open, in-progress, independent verification, resolved and controlled reopen states. Submission and closure require evidence references with SHA-256 integrity, and the submitter cannot verify their own remediation. Open high/critical or overdue findings block the existing release-gate control domain, remain visible in the executive portfolio and are included in the assurance dossier without invoking a model.

Migration `0064_fornost_ai_assurance_alerts.sql` turns approved baseline breaches into deterministic, persistent alerts for measurement freshness, accuracy, error rate, drift, bias, P95 latency and sample size. A SHA-256 fingerprint deduplicates recurring breaches while retaining occurrence count and latest evidence reference. Admins can assign, acknowledge, resolve and reopen alerts; one explicit confirmation converts a High/Critical alert into a source-linked CAPA. Open High/Critical alerts block the continuous-assurance release check and are included in the portfolio, CSV evidence and assurance dossier. Raw prompts and model responses are never stored in the alert record.

Migration `0065_fornost_ai_regulatory_obligations.sql` operationalizes regulatory profiles as accountable obligations across the EU AI Act, ISO/IEC 42001, NIST AI RMF, KVKK/GDPR and internal policy. Each obligation has a stable framework code, owner, separate reviewer, due date, priority, evidence plan and optional recurrence. Deterministic 30/15/7-day reminder windows and an overdue escalation queue are available in-app and as formula-safe CSV without disclosing evidence bodies. Implementation and independent verification require evidence references with SHA-256 integrity; creators and submitters cannot verify their own work. Completing a periodic obligation atomically closes the verified record and opens a linked recurring successor cycle from the later of the prior due date or verification date, preventing a late review from creating an already-overdue successor. Open High/Critical or overdue obligations fail the regulatory release-gate domain, appear in the executive action queue and are preserved in the assurance dossier. The workflow records references and hashes, not evidence bodies or sensitive model content.
