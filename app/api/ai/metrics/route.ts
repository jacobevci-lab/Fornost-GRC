...
export async function GET(req:NextRequest){
  ...
  const [activity,drafts,published,tickets,daily,models,errors,governance,evaluations,failover,agents,agentDrafts,knowledge]=await Promise.all([
    ...
    DB.prepare(`SELECT COUNT(*) total,SUM(CASE WHEN status='approved' THEN 1 ELSE 0 END) approved,SUM(CASE WHEN status='approved' THEN chunk_count ELSE 0 END) chunks FROM ai_knowledge_sources`).first<Record<string,unknown>>(),
  ]);
