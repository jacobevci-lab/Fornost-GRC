...
export function parseAiDraftResponse(kind: AiDraftKind, response: string) {
  ...
  for (const field of fields[kind]) {
    const value = cleanAiText(rawPayload[field], field === "description" || field === "recommendation" || field === "acceptanceCriteria" ? 1600 : 600);
  ...
  const title = cleanAiText(parsed.title || payload.title, 180);
  const rationale = cleanAiText(parsed.rationale, 1200);
