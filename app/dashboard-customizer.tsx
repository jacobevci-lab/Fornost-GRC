...
export default function DashboardCustomizer(){
  ...
  const apply=useCallback((target:HTMLElement|null,prefs:DashboardPreferences)=>{
    ...
    if(intelligence){
      const visibleIntelligence=WIDGETS.filter(widget=>widget.selector.includes("dashboard-intelligence")&&prefs.visible[widget.id]).length;
      intelligence.hidden=visibleIntelligence===0;
    }
  },[]);
  ...
  const toggle=(id:WidgetId)=>setPreferences(current=>({...current,preset:current.preset,visible:{...current.visible,[id]:!current.visible[id]}}));
    ...
    {open&&createPortal(<div className="dashboard-customizer-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}>
      <aside className="dashboard-customizer-drawer" role="dialog" aria-modal="true" aria-labelledby="dashboard-customizer-title">
        ...
        <section className="dashboard-customizer-summary">
          <div><small>{tr?"AKTİF PROFİL":"ACTIVE PROFILE"}</small><b>{preferences.preset==="executive"?(tr?"CISO / Executive":"CISO / Executive"):preferences.preset==="risk"?(tr?"Risk Odaklı":"Risk Focus"):preferences.preset==="assurance"?(tr?"Güvence Odaklı":"Assurance Focus"):(tr?"Operasyon":"Operations")}</b></div>
          <div><small>{tr?"GÖRÜNÜR WIDGET":"VISIBLE WIDGETS"}</small><b>{visibleCount}/{WIDGETS.length}</b></div>
          <div><small>{tr?"YOĞUNLUK":"DENSITY"}</small><b>{preferences.compact?(tr?"Kompakt":"Compact"):(tr?"Rahat":"Comfortable")}</b></div>
        </section>

        <section className="dashboard-customizer-section">
          <div className="dashboard-customizer-section-head"><div><small>01</small><h3>{tr?"Hazır yönetici profili":"Executive profile"}</h3></div><span>{tr?"Tek tıkla odak değiştirin":"Switch focus in one click"}</span></div>
          <div className="dashboard-preset-grid">
            {(["executive","risk","assurance","operations"] as PresetId[]).map(id=>{
              const labels={executive:tr?"CISO / Executive":"CISO / Executive",risk:tr?"Risk Odaklı":"Risk Focus",assurance:tr?"Güvence Odaklı":"Assurance Focus",operations:tr?"Operasyon":"Operations"};
              const descriptions={executive:tr?"Kurumsal risk + güvence + dayanıklılık":"Enterprise risk + assurance + resilience",risk:tr?"Risk yoğunluğu + karar kuyruğu":"Exposure + decision queue",assurance:tr?"Kontrol + kanıt + audit readiness":"Control + evidence + audit readiness",operations:tr?"Aksiyon + hızlı çalışma alanları":"Actions + quick workspaces"};
        ...
        <section className="dashboard-customizer-section">
          <div className="dashboard-customizer-section-head"><div><small>02</small><h3>{tr?"Widget görünürlüğü ve sırası":"Widget visibility & order"}</h3></div><span>{tr?"Göster, gizle ve önceliklendir":"Show, hide and prioritize"}</span></div>
        ...
        <section className="dashboard-customizer-section compact-setting">
          <div><small>03</small><span><b>{tr?"Kompakt yoğunluk":"Compact density"}</b><em>{tr?"Daha fazla karar bilgisini aynı ekranda gösterir.":"Shows more decision information on the same screen."}</em></span></div>
