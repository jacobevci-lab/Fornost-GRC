...
import FornostAiDataProtection from "./fornost-ai-data-protection";
import FornostAiModelInventory from "./fornost-ai-model-inventory";
import FornostAiCompliance from "./fornost-ai-compliance";
...
import FornostAiVendorAssurance from "./fornost-ai-vendor-assurance";
import FornostAiAccessGovernance from "./fornost-ai-access-governance";
import FornostAiReleaseGate from "./fornost-ai-release-gate";
import FornostAiImpactAssessment from "./fornost-ai-impact-assessment";
import FornostAiResilience from "./fornost-ai-resilience";
import FornostAiDatasets from "./fornost-ai-datasets";
import FornostAiRegulatory from "./fornost-ai-regulatory";
import FornostAiLiteracy from "./fornost-ai-literacy";
import FornostAiSupplyChain from "./fornost-ai-supply-chain";
import FornostAiRedTeam from "./fornost-ai-red-team";
import FornostAiTransparency from "./fornost-ai-transparency";
import FornostAiContinuousAssurance from "./fornost-ai-continuous-assurance";
import FornostAiPortfolio from "./fornost-ai-portfolio";
import FornostAiExceptions from "./fornost-ai-exceptions";
...
export default function FornostAiCopilot() {
  ...
  const [tab, setTab] = useState<"chat" | "portfolio" | "agents" | "knowledge" | "drafts" | "metrics" | "governance" | "models" | "compliance" | "lifecycle" | "incidents" | "evidence" | "risks" | "vendors" | "access" | "release" | "impact" | "resilience" | "datasets" | "regulatory" | "literacy" | "supply-chain" | "red-team" | "transparency" | "assurance" | "exceptions" | "decommission" | "feedback" | "budget" | "policy" | "protection" | "settings" | "audit">("chat");
  ...
  async function send(e: FormEvent) {
    ...
    setMessages((items) => [...items, {
      ...
      activityId:response.ok?String(body.activityId||""):undefined,
  ...
  function downloadMetrics(){if(!metrics)return;const rows=[["Bölüm","Metrik","Değer"],["Dönem","Gün",metrics.windowDays],["Aktivite","Toplam",metrics.activity.total],["Aktivite","Başarı oranı",`%${metrics.activity.successRate}`],["Aktivite","Hata",metrics.activity.errors],["Provider","Başarı oranı",`%${metrics.providerHealth.successRate}`],["Provider","Ortalama gecikme",`${metrics.providerHealth.averageLatencyMs} ms`],["Yönetişim","Geciken kullanım senaryosu",metrics.governance.overdue],["Değerlendirme","Dönem başarı oranı",`%${metrics.governance.evaluationPassRate}`],["Değerlendirme","Etkin testlerin son koşumu",`${metrics.governance.evaluationCurrent.passed}/${metrics.governance.evaluationCurrent.total}`],["Bilgi tabanı","Onaylı",metrics.knowledge.approved],["Bilgi tabanı","Geciken inceleme",metrics.knowledge.stale],["AI geri bildirim","Açık / incelemede",metrics.feedback.unresolved],["AI geri bildirim","Yüksek / kritik",metrics.feedback.highRisk],["AI kullanım","Aylık token",metrics.usage.tokens],["AI kullanım","Ölçümsüz çağrı",metrics.usage.unmetered],["AI kullanım","Limit aşımı",metrics.usage.exceeded],["Taslaklar","Bekleyen",metrics.drafts.pending],...metrics.controls.map(item=>["Kontrol",item.label,`${item.status}: ${item.detail}`])];const csv="\uFEFF"+rows.map(row=>row.map(csvCell).join(";")).join("\n"),url=URL.createObjectURL(new Blob([csv],{type:"text/csv;charset=utf-8"})),link=document.createElement("a");link.href=url;link.download=`fornost-ai-kalite-${metrics.windowDays}-gun.csv`;link.click();URL.revokeObjectURL(url);}
  ...
  const activeTab = user.role !== "Admin" && (tab === "portfolio" || tab === "settings" || tab === "audit" || tab === "metrics" || tab === "governance" || tab === "models" || tab === "compliance" || tab === "lifecycle" || tab === "incidents" || tab === "access" || tab === "release" || tab === "resilience" || tab === "datasets" || tab === "regulatory" || tab === "literacy" || tab === "supply-chain" || tab === "red-team" || tab === "transparency" || tab === "assurance" || tab === "exceptions" || tab === "decommission" || tab === "feedback" || tab === "budget" || tab === "policy" || tab === "protection") ? "chat" : tab;
  ...
  return <>
    ...
    {open && <section id="fornost-ai-panel" className="fornost-ai-panel" aria-label="Fornost AI Copilot">
      ...
      <nav className="fornost-ai-tabs">
        ...
        {user.role === "Admin" && <button className={activeTab === "compliance" ? "active" : ""} onClick={() => setTab("compliance")}>AI Uyum</button>}
        ...
        <button className={activeTab === "vendors" ? "active" : ""} onClick={() => setTab("vendors")}>AI Tedarikçi</button>
        {user.role === "Admin" && <button className={activeTab === "access" ? "active" : ""} onClick={() => setTab("access")}>AI Erişim</button>}
        {user.role === "Admin" && <button className={activeTab === "release" ? "active" : ""} onClick={() => setTab("release")}>AI Yayın Kapısı</button>}
        <button className={activeTab === "impact" ? "active" : ""} onClick={() => setTab("impact")}>AI Etki</button>
        {user.role === "Admin" && <button className={activeTab === "resilience" ? "active" : ""} onClick={() => setTab("resilience")}>AI Dayanıklılık</button>}
        {user.role === "Admin" && <button className={activeTab === "datasets" ? "active" : ""} onClick={() => setTab("datasets")}>AI Veri Setleri</button>}
        {user.role === "Admin" && <button className={activeTab === "regulatory" ? "active" : ""} onClick={() => setTab("regulatory")}>AI Regülasyon</button>}
        {user.role === "Admin" && <button className={activeTab === "literacy" ? "active" : ""} onClick={() => setTab("literacy")}>AI Yetkinlik</button>}
        {user.role === "Admin" && <button className={activeTab === "supply-chain" ? "active" : ""} onClick={() => setTab("supply-chain")}>AI Supply Chain</button>}
        {user.role === "Admin" && <button className={activeTab === "red-team" ? "active" : ""} onClick={() => setTab("red-team")}>AI Red Team</button>}
        {user.role === "Admin" && <button className={activeTab === "transparency" ? "active" : ""} onClick={() => setTab("transparency")}>AI Şeffaflık</button>}
        {user.role === "Admin" && <button className={activeTab === "assurance" ? "active" : ""} onClick={() => setTab("assurance")}>AI Güvence</button>}
      ...
      : activeTab === "vendors" ? <FornostAiVendorAssurance role={user.role}/>
      : activeTab === "access" ? <FornostAiAccessGovernance/>
      : activeTab === "release" ? <FornostAiReleaseGate/>
      : activeTab === "impact" ? <FornostAiImpactAssessment role={user.role}/>
      : activeTab === "resilience" ? <FornostAiResilience/>
      : activeTab === "datasets" ? <FornostAiDatasets/>
      : activeTab === "regulatory" ? <FornostAiRegulatory/>
      : activeTab === "literacy" ? <FornostAiLiteracy/>
      : activeTab === "supply-chain" ? <FornostAiSupplyChain/>
      ...
      : activeTab === "protection" ? <FornostAiDataProtection/>
