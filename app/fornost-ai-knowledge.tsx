...
export default function FornostAiKnowledge({role}:{role:Role}){
  ...
  async function openDetail(id:string){setBusy(true);const body=await fetchDetail(id);if(body)setDetail(body);setBusy(false);}
  async function prepareVersion(source:KnowledgeSource){setBusy(true);const body=await fetchDetail(source.id);if(body){setDetail(body);setAction({id:source.id,kind:"version",note:"",content:body.content,confirmation:""});}setBusy(false);}
  ...
  async function search(event:FormEvent){event.preventDefault();if(query.trim().length<3||searchBusy)return;setSearchBusy(true);setNotice("");const response=await fetch(withBasePath("/api/ai/knowledge/search"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({query:query.trim()})}).catch(()=>null),body=await response?.json().catch(()=>({}))||{};if(response?.ok)setMatches(Array.isArray(body.matches)?body.matches:[]);else setNotice(String(body.error||"Retrieval testi çalıştırılamadı."));setSearchBusy(false);}
