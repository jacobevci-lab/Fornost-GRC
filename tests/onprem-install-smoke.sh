...
make_case() {
  ...
  printf 'FORNOST_BASE_PATH=%s\nFORNOST_HTTPS_PORT=%s\nFORNOST_SETTINGS_ENCRYPTION_KEY=%s\nFORNOST_DOSSIER_SIGNING_KEY=%s\n' "${base_path}" "${port}" "test-only-settings-encryption-key-0123456789abcdef0123456789abcdef" "test-only-dossier-signing-key-0123456789abcdef0123456789abcdef" >"${project_dir}/.env.onprem"
