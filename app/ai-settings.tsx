...
export default function AiSettings({lang}:{lang:Lang}){
  const tr=lang==="tr",[settings,setSettings]=useState(defaults),[busy,setBusy]=useState(false),[notice,setNotice]=useState(""),[models,setModels]=useState<string[]>([]),[available,setAvailable]=useState<boolean|null>(null);
  const load=useCallback(async()=>{setBusy(true);const response=await fetch(withBasePath("/api/ai/providers"),{cache:"no-store"}).catch(()=>null),body=await response?.json().catch(()=>({}))||{};if(response?.ok)setSettings({...defaults,...body,secret:"",fallback:{...defaults.fallback,...(body.fallback||{}),secret:""}});else setNotice(String(body.error||(tr?"AI ayarları okunamadı.":"AI settings could not be loaded.")));setBusy(false);},[tr]);
