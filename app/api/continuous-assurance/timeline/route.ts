...
export async function GET(req:NextRequest){
  ...
  for(const row of escalations){
    const reference=String(row.subject_ref||row.id||""),kind=String(row.kind||"assurance-escalation"),severity=String(row.severity||"medium"),title=String(row.title||"Assurance escalation"),rawSource=parse(String(row.source_json||"{}")),findingCode=findingCodeById.get(String(rawSource.findingId||""))||String(rawSource.findingCode||""),source=findingCode?{...rawSource,findingCode}:rawSource,navigation=assuranceEscalationNavigation(kind,source),nav=navigation?navigation:{};
